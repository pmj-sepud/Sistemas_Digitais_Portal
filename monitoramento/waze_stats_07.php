  ...
  <div class="col-sm-12">
      <?
            $sql = "SELECT
                     ...
                     EXTRACT(DAY FROM start_time AT TIME ZONE '0') AS dia_local,
                     EXTRACT(MONTH FROM start_time AT TIME ZONE '0') AS mes_local
                  FROM
                      	waze.data_files
                  WHERE
                      	    start_time AT TIME ZONE '0' BETWEEN '{$agora['ano']}-{$agora['mes']}-01 00:00:00' AND '{$agora['ano']}-{$agora['mes']}-{$agora['ultimo_dia']} 23:59:59'
            ...
            while($d = pg_fetch_assoc($res))
            {
              ...
              //  $vet2['Atraso'][$minutos]  = round(($d['jams_delay']/60),1);
                ...
                $legenda[] = "'".$d['dia_local']."-".$hora."'";

                $vet2['Atraso'][]  = ceil($d['jams_delay']/60);
                $vet2['Comprimento'][]  = round(($d['jams_length']/1000),1);

                if(ceil($d['jams_delay']/60)         >= $maxdelay) { $maxdelay  = ceil($d['jams_delay']/60);          $maxdelayTime  = $d['dia_local']."/".$d['mes_local']." - ".$hora; }
                if(round(($d['jams_length']/1000),1) >= $maxlenght){ $maxlenght = round(($d['jams_length']/1000),1);  $maxlenghtTime = $d['dia_local']."/".$d['mes_local']." - ".$hora; }
