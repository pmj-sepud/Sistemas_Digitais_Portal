...
require("../libs/php/conn.php");
      ...
      $sql = "SELECT
            ...
            FROM
                  waze.data_files
            WHERE
                      start_time AT TIME ZONE '0' BETWEEN '".$agora['datasrv']." 12:00:00' AND '".$agora['datasrv']." 23:59:59'
      ...
      while($d = pg_fetch_assoc($res))
      {
          ...
          $atualdelay  = ceil($d['jams_delay']/60);         $atualdelayTime  = str_pad($d['hora_local'],2,"0",STR_PAD_LEFT).":".str_pad($d['minuto_local'],2,"0",STR_PAD_LEFT);
          $atuallenght = round(($d['jams_length']/1000),1); $atuallenghtTime = str_pad($d['hora_local'],2,"0",STR_PAD_LEFT).":".str_pad($d['minuto_local'],2,"0",STR_PAD_LEFT);
      }

      if(isset($vet2['Atraso']))      { $serieAtraso  = "{name: 'Atraso (Minutos)', data: [".implode(",",$vet2['Atraso'])."]}";     }
      if(isset($vet2['Comprimento'])){ $serieTamanho = "{name: 'Comprimento (Km)', data: [".implode(",",$vet2['Comprimento'])."]}"; }

      if(!isset($vet2['Atraso']) && !isset($vet2['Comprimento']))
      {
        echo "<h3>Não há informações para este período.</h3>";
        echo "<h5>Waze - Evolução do congestionamento e atraso no perído da tarde</h5>";
        $timeToChange = 10;
        exit();
      }

?>
<div  style="position:absolute;z-index:10;margin-top:55px;margin-left:5px"><img src="../assets/images/waze-logo.png" width="200px"></div>
<div class='row'>
  <div class="col-sm-12">

      <div id="graf" style="width:100%; height:500px;margin-top:50px"></div>
      <script>
...
<div class='row' style="margin-top:20px">
  ...
  <div class="col-sm-6" align="center">
      <span><i>Somatória dos congestionamentos</i></span><br>
      <span style="font-size:35px;">Pico da tarde:</span>
